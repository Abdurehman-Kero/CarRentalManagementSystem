...
const MainLayout = () => {
  ...
  return (
    <div className="flex h-screen bg-surface-950 text-surface-100 w-full overflow-hidden">
      {/* ── Mobile overlay ── */}
      {sidebarOpen && (
        <div
      ...
      <aside
        ...
        style={{
          background:
            "linear-gradient(180deg, #05070B 0%, #111827 50%, #05070B 100%)",
        }}
      >
        {/* Brand */}
        <div className="flex items-center gap-3 h-16 px-5 border-b border-surface-900 flex-shrink-0">
          <div
            ...
            style={{
              background: "linear-gradient(135deg, #FFEAA7 0%, #C39348 100%)",
              boxShadow: "0 4px 12px rgba(195,147,72,0.4)",
            }}
          >
            <svg
              className="w-5 h-5 text-surface-950"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div>
            ...
            <p className="text-[10px] text-primary-400 mt-0.5 uppercase tracking-[0.2em] font-semibold">
              Luxury Portal
            </p>
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-5 px-3 space-y-0.5">
          ...
          {navigation.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`flex-shrink-0 ${active ? "text-surface-950" : "text-surface-500"}`}
                >
                  {item.icon}
                </span>
                <span className={active ? "text-surface-950 font-bold" : ""}>
                  {item.name}
                </span>
                {active && (
          ...
          {isSuperAdmin && (
            <>
              ...
              >
                <span
                  className={`flex-shrink-0 ${location.pathname === "/dashboard/admins" ? "text-surface-950" : "text-surface-500"}`}
                >
                  <svg
                    className="w-5 h-5"
                    fill="none"
        ...
        <div className="flex-shrink-0 px-3 py-4">
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-surface-900 border border-surface-800">
            <div
              className="w-8 h-8 rounded-full flex items-center justify-center text-surface-950 text-xs font-bold flex-shrink-0"
              style={{
                background: "linear-gradient(135deg, #D5B277, #846029)",
              }}
            >
              {initials}
            </div>
            ...
            <button
              ...
              title="Sign out"
              className="flex-shrink-0 p-1.5 rounded-lg text-surface-400 transition-colors hover:bg-surface-800 hover:text-surface-100"
            >
              <svg
                className="w-4 h-4"
                fill="none"
      ...
      {/* ══════════════════════════════════
          MAIN AREA
      ══════════════════════════════════ */}
      <div className="flex-1 flex flex-col min-w-0 bg-surface-950 overflow-y-auto">
        {/* Top bar */}
        <header
          className="sticky top-0 z-30 flex items-center gap-4 h-14 sm:h-16 px-3 sm:px-6
        ...
        >
          ...
          <div className="flex items-center gap-2 text-sm">
            <span className="text-surface-500 hidden sm:block text-xs">
              Pages
            </span>
            <svg
              className="w-3 h-3 text-surface-800 hidden sm:block"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            {isSuperAdmin && (
              <span
                ...
                style={{
                  background: "rgba(195,147,72,0.1)",
                  color: "#DFBA73",
                  border: "1px solid rgba(195,147,72,0.2)",
                }}
              >
                <svg
                  className="w-3 h-3"
            ...
            {/* Date */}
            <div
              className="hidden md:flex items-center gap-1.5 text-xs text-surface-300 font-medium
            ...
            <button
              type="button"
              onClick={toggleTheme}
              className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center text-surface-400 hover:text-primary-400 transition-colors border border-surface-800 hover:border-primary-500/30"
            ...
            <div className="relative">
              <button
                ...
                className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center text-surface-950 text-xs sm:text-sm font-bold
              ...
              {profileOpen && (
                <div
                  className="absolute right-0 top-full mt-2 w-52 bg-surface-900 rounded-2xl shadow-modal
                ...
                >
                  <div className="px-4 py-3 border-b border-surface-800">
                    ...
                    <span
                      className="inline-flex mt-1 items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold"
                      style={{
                  ...
                  <button
                    ...
                    className="flex items-center gap-2.5 w-full px-4 py-3 text-sm font-semibold
                               text-danger-500 hover:text-danger-600 hover:bg-danger-50/10 transition-colors"
                  >
                    <svg
                      className="w-4 h-4"
                      fill="none"
